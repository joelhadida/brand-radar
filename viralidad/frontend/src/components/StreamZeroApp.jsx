...
import { ChannelScraper } from './ChannelScraper';

export function StreamZeroApp({ onBack }) {
  const { job, error, isSubmitting, startJob, clearJob } = useJob();
      ...
      <header className="sticky top-0 z-10 border-b border-slate-800/60 bg-slate-950/90 backdrop-blur-sm">
        <div className="max-w-5xl mx-auto px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-violet-600/20 border border-violet-500/30 flex items-center justify-center text-base">
              📺
            </div>
            <div>
              <h1 className="text-base font-bold text-white leading-none">Momentio TV</h1>
              <p className="text-xs text-slate-500 mt-0.5">AI-powered viral clip engine</p>
            </div>
          </div>
        </div>
      </header>
          ...
          <>
            ...
            {!job && !isProcessing && !isCompleted && (
              <div className="text-center py-16 text-slate-400">
                <p className="text-sm">Ingresa una URL de YouTube para comenzar</p>
              </div>
            )}
          </>
        )}
